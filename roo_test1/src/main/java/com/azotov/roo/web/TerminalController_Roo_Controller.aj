...
privileged aspect TerminalController_Roo_Controller {
    ...
    public String TerminalController.create(@Valid Terminal terminal, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, terminal);
    ...
    public String TerminalController.createForm(Model uiModel) {
        populateEditForm(uiModel, new Terminal());
        return "terminals/create";
    }
    ...
    public String TerminalController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("terminals", Terminal.findTerminalEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Terminal.countTerminals() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("terminals", Terminal.findAllTerminals(sortFieldName, sortOrder));
        }
    ...
    public String TerminalController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, Terminal.findTerminal(id));
        return "terminals/update";
    }
    ...
    void TerminalController.populateEditForm(Model uiModel, Terminal terminal) {
